...
Recollectium Core is now at v1.2.0. The v1.0.0 section below is historical and records the local memory core, CLI, Python API, local HTTP API, MCP stdio, managed MCP service, local embeddings, service lifecycle, install, upgrade, uninstall, logging, shell completion, workspace UID management, configuration, and adapter discovery contract that shipped in the initial release.
...
- [x] Inline re-embedding with embedding job tracking, explicit refresh commands, and job audit-record cleanup across CLI, API, and MCP.
...
- [x] Install-time and upgrade-time embedding maintenance prepares the configured model and refreshes stale or missing embeddings when needed.
...
- [x] Ship the dedicated spawnable WebUI control plane for browser-based memory, config, service, embedding, dev, graph, diagnostics, and logs workflows.
- [ ] Add explicit admin copy/move migration from legacy `database.path` installs into the new folder layout; v1.2 now rejects legacy path configs instead.
