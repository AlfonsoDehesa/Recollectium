...
- Human-readable output is the default. When the target output stream is a TTY, human-readable output uses ANSI color for headings, field labels, errors, and hints. Set `cli_output` to `json` in config, or pass `--json`, when scripts or adapters need machine-readable output.
