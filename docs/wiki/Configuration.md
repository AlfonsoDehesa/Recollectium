...
| `cli_output` | string | `human_readable` | `human_readable`, `json` | Output format for CLI command results and non-argparse failures. Use `human_readable` for terminal-friendly summaries and errors. Human-readable output is ANSI-colored only when the target stream is a TTY; pipes and captured output remain plain text. Use `json` for scripts and adapters. |
